# PharmaVoice – Google Play Store Listing (Copy-&-Paste-fertig)

Alle Felder unten sind direkt in die Play Console kopierbar. Zeichen-Limits sind angegeben.
Kein Heilversprechen – die App ist ein Lernwerkzeug.

**Wichtige URLs (bereits online):**
- Datenschutz: `https://leofernandesgaspar92-commits.github.io/pharmavoice/datenschutz.html`
- Impressum:   `https://leofernandesgaspar92-commits.github.io/pharmavoice/impressum.html`
- App/Website: `https://leofernandesgaspar92-commits.github.io/pharmavoice/`

**Unternehmensdaten (für Konto-Verifizierung):**
- Rechtlicher Name: Leo Fernandes Gaspar (Einzelunternehmer)
- D-U-N-S: `303262490` · GISA: `40028903`
- Kontakt: leofernandesgaspar92@gmail.com

---

## App-Name  *(max. 30 Zeichen)*
```
PharmaVoice: Wirkstoffe
```
EN: `PharmaVoice: Drug Names` · PT: `PharmaVoice: Fármacos`

---

## Kurzbeschreibung  *(max. 80 Zeichen)*

**DE** (78)
```
Aussprache von 440 Wirkstoffen lernen – spielerisch, offline, in 3 Sprachen.
```
**EN** (76)
```
Learn to pronounce 440 drug names – gamified, offline, in three languages.
```
**PT** (77)
```
Aprenda a pronunciar 440 fármacos – gamificado, offline, em três idiomas.
```

---

## Vollständige Beschreibung  *(max. 4000 Zeichen)*

### DE
```
Sprich medizinische Wirkstoffnamen endlich richtig aus – ohne Stolpern, ohne Unsicherheit.

PharmaVoice bringt dir die korrekte Aussprache von 440 gängigen Wirkstoffen bei: Silbe für Silbe, mit Betonung, Lautschrift und Sprachausgabe zum Nachsprechen. Ideal für PKA-/PTA-Lehrlinge, Apothekenpersonal, Pharmareferent:innen und Pharmazie-Studierende.

WARUM PHARMAVOICE?
• 440 Wirkstoffe mit Silbentrennung, Betonung und Lautschrift (IPA)
• Hören & Nachsprechen – die richtige Betonung sitzt Silbe für Silbe
• Wirkstoff-Lexikon mit Indikation, Wirkstoffklasse und ATC-Code
• Suchen, filtern und alphabetisch sortieren

SPIELERISCH LERNEN
• Tägliches Lernziel, XP und Level
• Lernserie („Streak") – bleib jeden Tag am Ball
• Spaced Repetition: die App wiederholt genau das, was du auffrischen musst
• Belohnungen, Kombos und Level-Aufstiege

KLARES, MODERNES DESIGN
• Helles und dunkles Design (frei umschaltbar)
• Übersichtlich, ablenkungsfrei, auf Lesbarkeit optimiert

DEINE DATEN BLEIBEN BEI DIR
• Fortschritt wird lokal auf deinem Gerät gespeichert
• Funktioniert offline
• Kein Tracking, keine Werbung

DREISPRACHIG
Deutsch, Englisch und Portugiesisch – Oberfläche und Fachtexte.

Hinweis: PharmaVoice ist ein Lernwerkzeug und ersetzt keine medizinische oder pharmazeutische Beratung.
```

### EN
```
Finally pronounce medical drug names correctly – no stumbling, no doubt.

PharmaVoice teaches you the correct pronunciation of 440 common drug substances: syllable by syllable, with stress marks, phonetic transcription and audio to repeat after. Perfect for pharmacy assistants and technicians, pharmacy staff, pharma reps and pharmacology students.

WHY PHARMAVOICE?
• 440 drugs with syllable breakdown, stress and phonetics (IPA)
• Listen & repeat – nail the right stress, syllable by syllable
• Drug encyclopedia with indication, drug class and ATC code
• Search, filter and sort alphabetically

LEARN THE FUN WAY
• Daily goal, XP and levels
• Learning streak – keep it up every day
• Spaced repetition: the app reviews exactly what you need to refresh
• Rewards, combos and level-ups

CLEAN, MODERN DESIGN
• Light and dark theme (freely switchable)
• Clear, distraction-free, optimised for readability

YOUR DATA STAYS WITH YOU
• Progress is stored locally on your device
• Works offline
• No tracking, no ads

THREE LANGUAGES
German, English and Portuguese – interface and content.

Note: PharmaVoice is a learning tool and does not replace medical or pharmaceutical advice.
```

### PT
```
Pronuncie corretamente os nomes de fármacos – sem tropeços, sem dúvidas.

O PharmaVoice ensina a pronúncia correta de 440 substâncias comuns: sílaba a sílaba, com sílaba tônica, transcrição fonética e áudio para repetir. Ideal para auxiliares e técnicos de farmácia, equipe de farmácia, representantes farmacêuticos e estudantes de farmácia.

POR QUE PHARMAVOICE?
• 440 fármacos com divisão silábica, tonicidade e fonética (IPA)
• Ouça e repita – acerte a sílaba tônica, sílaba a sílaba
• Enciclopédia com indicação, classe e código ATC
• Busque, filtre e ordene em ordem alfabética

APRENDA DE FORMA DIVERTIDA
• Meta diária, XP e níveis
• Sequência de estudo – mantenha todo dia
• Repetição espaçada: o app revisa exatamente o que você precisa
• Recompensas, combos e subidas de nível

DESIGN MODERNO E LIMPO
• Tema claro e escuro (alternável)
• Claro, sem distrações, otimizado para leitura

SEUS DADOS FICAM COM VOCÊ
• Progresso salvo localmente no seu dispositivo
• Funciona offline
• Sem rastreamento, sem anúncios

TRÊS IDIOMAS
Alemão, inglês e português – interface e conteúdo.

Observação: o PharmaVoice é uma ferramenta de aprendizagem e não substitui aconselhamento médico ou farmacêutico.
```

---

## Kategorie & Einstufung
- **Kategorie:** Bildung (Education)
- **Tags:** Bildung, Medizin, Sprachen lernen, Karteikarten
- **Inhaltseinstufung:** Für alle Altersgruppen. Im Fragebogen ehrlich: keine Gewalt, keine
  nutzergenerierten Inhalte, kein Standort, keine sensiblen Daten.
- **Enthält Werbung:** Nein
- **In-App-Käufe:** derzeit Nein (falls später Freemium/Abo aktiv → auf Ja ändern)

---

## Data-Safety-Formular (aktueller Stand)
- **Werden Daten erfasst/geteilt?** Nein – alle Daten bleiben lokal auf dem Gerät (localStorage).
- **Keine Übertragung an Server, kein Tracking, keine Werbe-ID.**
- Mikrofon: wird nur lokal im Gerät zur Aussprache-Erkennung genutzt, es wird keine Aufnahme
  gespeichert oder übertragen (im Fragebogen entsprechend angeben).
- ⚠️ Nach Einführung von Firebase/Cloud-Login oder Google Play Billing **anpassen**.

---

## Keyword-Hinweise (fließen in Kurz-/Langtext ein)
`Wirkstoff, Aussprache, Pharmazie, PKA, PTA, Apotheke, Medikamente lernen, Lautschrift, ATC, Pharmakologie`
(EN: `drug pronunciation, pharmacy, pharmacology, medical terms, flashcards`)

---

## Grafik-Assets
| Asset | Vorgabe | Status |
|---|---|---|
| App-Icon | 512×512 PNG | vorhanden (`icon-512.png`) |
| Feature-Grafik | 1024×500 | ✅ `store-assets/feature-graphic.png` (aktualisiert, „440") |
| Screenshots Handy | mind. 2 (empf. 4–8), 9:16 | ✅ `store-assets/screenshots/01-home … 05-profil.png` (1170×2532, aktuelles helles Design) |
| (optional) Kurzvideo | YouTube-Link | optional |

Reihenfolge-Empfehlung im Store: **01-home → 02-quiz → 03-lexikon → 04-result → 05-profil**.

---

## Pre-Launch-Checkliste (Store)
- [x] Datenschutzerklärung öffentlich unter eigener URL
- [x] Impressum-/Kontaktangaben (Wohnadresse durch Platzhalter ersetzt)
- [x] Feature-Grafik + 5 aktuelle Screenshots vorhanden
- [x] Play-Entwicklerkonto (Organisation) verifiziert – D-U-N-S ✓, Zahlung ✓, Website-Inhaberschaft ✓ (Search Console)
- [ ] Signierter `.aab`-Upload (lokal, siehe CAPACITOR_SETUP.md) + Keystore sicher sichern
- [ ] Data-Safety-Formular ausgefüllt (siehe oben)
- [ ] Inhaltseinstufungs-Fragebogen ausgefüllt
- [ ] Interner Test → Produktion, Release eingereicht
